...
const Loader = ({
  ...
  const wrapperClassName = clsx(
    ...
    fullScreen
      ...
      : "py-2",
    overlay && "absolute inset-0 z-20 rounded-[inherit] bg-white/80 backdrop-blur-sm",
    className
  );
  ...
  return (
    <div className={wrapperClassName} role="status" aria-live="polite">
      ...
      <div className="text-center">
        {showBrand && (
          <p className="text-sm font-semibold tracking-[0.24em] text-[#0b1e43] uppercase">
            TrustScore
          </p>
        )}
        <p className={clsx("text-sm text-slate-500", showBrand && "mt-1")}>{label}</p>
      </div>
    </div>
  );
};
